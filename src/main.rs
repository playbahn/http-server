...
fn main() {
    ...
    let logbuf: Arc<Mutex<BufWriter<File>>> = Arc::new(Mutex::new(BufWriter::new(
        match File::options().append(true).create(true).open("server.log") {
            Ok(file) => file,
    ...
    match logbuf.lock() {
        Ok(mut logbuf) => {
            if let Err(e) =
                logbuf.write_all(format!("[{}] SERVER INIT\n", server_init_timestamp).as_bytes())
            {
            ...
            println!("[{}] SERVER INIT", server_init_timestamp);
        }
    ...
    ctrlc::set_handler(move || {
        ...
        let mut sigint_logbuf: MutexGuard<'_, BufWriter<File>> = match sigint_logbuf.lock() {
            Ok(logbuf) => logbuf,
            Err(poison_err) => {
                eprintln!("ctrlc_handler: BufWriter<server.log> PoisonError. Logging nonetheless.");
                poison_err.into_inner()
            },
        };

        // connect to same socket our server is listening on, so that we send a shutdown request,
        // after obviously setting loop condition variable [`sigint_retrieve`] to false
        match TcpStream::connect(server_socket_addr) {
            Ok(mut send_stream) => {
                let shutdown_request: &str = "SERVER SHUTDOWN REQUEST SENT";
                ...
                match send_stream.write_all(
                    ...
                    Ok(()) => {
                        ...
                        let shutdown_request_success_log: String =
                            format!("ctrlc_handler: {shutdown_request}\n");

                        if let Err(e) = sigint_logbuf.write_all(
                            format!("[{shutdown_request_success_timestamp}] {shutdown_request_success_log}").as_bytes()
                        ) {
                            eprintln!("ctrlc_handler: ERROR LOGGING DUPE REQUEST MESSAGE: {:#?}", e.kind());
                        } else if let Err(e) = sigint_logbuf.flush() {
                            eprintln!("ctrlc_handler: FLUSH ERROR WHILE LOGGING SERVER SHUTDOWN REQUEST: {:#?}", e.kind());
                        };
                    ...
                    Err(e) => {
                        ...
                        let shutdown_request_failure_log: String = format!(
                            "[{shutdown_request_failure_timestamp}] ctrlc_handler: Couldn't send server shutdown request: {:#?}\n", e.kind()
                        );

                        if let Err(e) = sigint_logbuf.write_all(
                            shutdown_request_failure_log.as_bytes()
                        ) {
                            eprintln!("ctrlc_handler: ERROR LOGGING FAILED DUPE REQUEST MESSAGE: {:#?}", e.kind());
                        } else if let Err(e) = sigint_logbuf.flush() {
                            eprintln!("ctrlc_handler: FLUSH ERROR WHILE LOGGING FAILED SERVER SHUTDOWN REQUEST: {:#?}", e.kind());
                        };
            ...
            Err(e) => {
                ...
                let localhost_conn_err_log: String = format!("[{localhost_conn_err_timestamp}] ctrlc_handler: ERROR CONNECTING TO LOCALHOST: {:#?}\n", e.kind());
    ...
    }).expect("Error setting ctrlc_handler.");
        ...
        let stream: TcpStream = match stream {
            ...
            Err(e) => {
                ...
                let err: String = format!(
                    "[{tcp_stream_encountered_io_timestamp}] ENCOUNTERED IO ERROR: {:#?}",
                    e.kind()
                );
...

                {
                    ...
                    if let Err(e) =
                        logbuf.write_all(format!("{err} in incoming TcpStream\n").as_bytes())
                    {
                        eprintln!(
                            "Incoming TcpStream encountered IO error LOG ERROR: {:#?}",
                            e.kind()
                        );
                    } else if let Err(e) = logbuf.flush() {
                        eprintln!(
                            "Incoming TcpStream encountered IO error LOG WRITE FLUSH ERROR: {:#?}",
                            e.kind()
                        );
                    };
                }
    ...
    let mut logbuf: MutexGuard<'_, BufWriter<File>> = match logbuf.lock() {
        ...
        Err(poison_err) => {
            eprintln!(
                "Connection handler: BufWriter<server.log> PoisonError. Logging nonetheless."
            );
            poison_err.into_inner()
        }
    };
    ...
    if let Err(e) =
        logbuf.write_all(format!("[{}] SERVER SHUTDOWN\n\n", server_init_timestamp).as_bytes())
    {
...
fn handle_connection(
    ...
    logbuf: Arc<Mutex<BufWriter<File>>>,
    ...
    match http_request.iter().peekable().peek() {
        ...
        Some(request_line) => {
            let (status_line, filename): (&str, &str) = match &request_line[..] {
                ...
                "CHECKED BY SERVER: 500 Internal Server Error" => {
...

            {
                ...
                if let Err(e) = logbuf.write_all(
                    ...
                    eprintln!(
                        "Connection handler: HTTP REQUEST LOG ERROR: {:#?}",
                        e.kind()
                    );
                } else if let Err(e) = logbuf.flush() {
                    eprintln!(
                        "Connection handler: HTTP REQUEST LOG WRITE FLUSH ERROR: {:#?}",
                        e.kind()
                    );
                };

                println!("[{request_timestamp}] Connection handler: Request:");

                for line in &http_request {
                    if let Err(e) = logbuf.write_all(format!("{line}\n").as_bytes()) {
                        eprintln!(
                            "Connection handler: HTTP REQUEST LOG ERROR: {:#?}",
                            e.kind()
                        );
                ...
                match stream.write_all(response.as_bytes()) {
                    Ok(()) => {
                        ...
                        let response_ok: String = format!(
                            "[{response_timestamp}] Connection handler: Response:\n{}\n",
                            response
                        );

                        if let Err(e) = logbuf.write_all(response_ok.as_bytes()) {
                            eprintln!(
                                "Connection handler: HTTP RESPONSE LOG ERROR: {:#?}",
                                e.kind()
                            );
                        } else if let Err(e) = logbuf.flush() {
                            eprintln!(
                                "Connection handler: HTTP RESPONSE LOG WRITE FLUSH ERROR: {:#?}",
                                e.kind()
                            );
                        };
                    ...
                    Err(e) => {
                        ...
                        if let Err(e) = logbuf.write_all(response_err.as_bytes()) {
                            eprintln!(
                                "Connection handler: HTTP FAILED RESPONSE LOG ERROR: {:#?}",
                                e.kind()
                            );
                        } else if let Err(e) = logbuf.flush() {
